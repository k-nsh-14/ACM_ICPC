...
def travel(room_info,room_size, point):
	...
	print room_size
	print point
	print room[point[0]][point[1]]
	if room[point[0]][point[1]] != "#":
		if point not in black_tile_list:
			black_tile_list.append(point)
		for next in next_pattern:
			next_point = (point[0]+next[0], point[1]+next[1])
			if (0 <= next_point[0] < room_size[0]) and (0 <= next_point[1] < room_size[1]):
...
for line in data:
	...
	if define_flag == 0:
		...
		room_info = map(int, line[:-2].split())
		define_flag = int(room_info[1])
		room = []
		black_tile_list = []
	else:
